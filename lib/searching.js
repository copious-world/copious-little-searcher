...
function default_string_hash(string) {
    let hash = 0;
    if (string.length === 0) return hash;
    let output = string.split('').reduce((hash, char) => {
        let prev_hash = hash;
        hash ^= char.charCodeAt(0)
        return char.charCodeAt(0) + (hash << 6) + (prev_hash << 16) - prev_hash;
    }, 0);
...
class Searching extends Registry {
    ...
    constructor(conf,QueryInterfaceClass) {
        ...
        super(conf)
        //
        this.hash_func = default_string_hash
     ...
     */
    query_hasher(query) {
        if ( typeof this.hash_func === 'function' ) {
            let s = query.trim()
    ...
    _run_query(match_text,orderby,qry) {
        ...
        if ( match_text === 'any' ) {
        ...
        } else if ( qry.op_is_stored_function(orderby) ) {
            ...
            try {
                ...
                results = this.global_file_list.filter((object) => {
                    return self.good_match(object,match_text)
                })
                //
                switch ( orderby ) {
                    case 'update_date' :  {
